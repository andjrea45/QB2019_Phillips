...
rm(list=ls())
getwd()
setwd("C:/Users/andjr/Documents/GitHub/QB2019_Phillips/2.Worksheets/3.RStudio")
```
...
> Answer 1:It generates a set of random numbers of a normal distribution that fit the specified mean and standard deviation. 
...
> Answer 3: It seems that there is a general positive trend between the zooplankton biomass and the TIN, SRP, TN, and TP, but not with between the zooplankton biomass and the chlorophyll a concentration. In fact, chlorophyll a has almost no variance across all observations, so there isn't anything interesting to show in relation to chlorophyll a concentration. The total phosphorous concentration has a positive correlation with total nitrogen concentration, the soluble reactive phosphorous concentration, the total inorganic nutrient concentration, and the zooplankton biomass.
...
> Answer 4: Yes, the p values changed depending on whether parametric or non-parametric methods were used. Specifically, using parametric methods made the p values much lower when looking at correlations between different values. This shows that the false discovery rate is higher when multiple comparisons are used. With a sample size of only 24, the assumptions of the central limit theorem are not met, and we're more likely to get false discoveries using parametric methods. 
False discovery rate is important, because as we study biodiversity it is important that the limited efforts (there is a lot of nature and not a lot of biodiversity researchers) of researchers be focused in the best places--false discoveries may lead to false allocation of resources.
...
matlines(newTN, conf95[, c("lwr", "upr")], type="l", lty = 2, lws = 1, col = "black")
...
> Answer 5: This regression supports what the data showed previously, that zooplankton biomass and total nitrogen are positively correlated. At low nitrogen concentration, biomass is low, at medium concentration biomass is medium, and at high concentration biomass is high. 
...
```{r}
meso <- read.table("data/zoops.txt", header = TRUE)
str(meso)
meso.num <- meso[, 3:8]
meso.num
```
